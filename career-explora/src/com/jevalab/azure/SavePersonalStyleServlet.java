...
public class SavePersonalStyleServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if (!session.isNew()) {
			...
			boolean updated = Util.updatePersonalStyles(session,
					StringConstants.PERSONAL_STYLE, testDate, new Text(styles));

			UserProfile up = null;
			if (updated) {

				up = Util.getUserProfileFromSession(session);
				List<String> list = up.getStyles();
				Map<String, Object> map = new HashMap<>();

				if (list.size() > 6) {
					list = up.getStyles().subList(0, 6);
